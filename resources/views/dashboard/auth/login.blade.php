    ...
    <div class="login-content flex-row-fluid d-flex flex-column justify-content-center position-relative overflow-hidden p-7 mx-auto">
        <div class="d-flex flex-column-fluid flex-center">
            <div class="login-form login-signin">
                <form method="POST" action="{{   (request()->is('manager/*')) ? route('manager.auth.login') : route('admin.auth.login')}}" class="form" novalidate="novalidate">
                    @csrf
                    ...
                    {{-- <a href="{{ (request()->is('manager/*')) ? route('manager.auth.password.request') : route('admin.auth.password.request') }}" class="forgot-pass">هل نسيت كلمة السر</a><br> --}}
                   @if (request()->is('manager/*'))
                   <a href="/admin/auth/login" class="forgot-pass">تسجيل الدخول كمدير نظام</a><br>
                   @else

                   <a href="/manager/auth/login" class="forgot-pass">تسجيل الدخول كمدير متجر</a><br>
